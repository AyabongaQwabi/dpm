...
export const alt = 'ServicePros city'
...
export default async function Image({ params }: { params: Promise<{ location: string }> }) {
  ...
  const { total } = await getPublishedProvidersPage(supabase, { city: location, page: 1, pageSize: 1 })
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div style={{ display: 'flex', fontSize: 30, fontWeight: 700, color: colors.accent }}>
            {total > 0 ? `${total} verified provider${total === 1 ? '' : 's'}` : 'South Africa'}
          </div>
        </div>
      </div>
    ),
